...
impl Config {
    pub fn new() -> Self {
        let api_key = env::var("COHERE_API_KEY").unwrap_or_else(|_| {
            println!("{}", "This program requires an Co:Here API key to run. Please set the COHERE_API_KEY environment variable.".red());
            exit(1);
        });
